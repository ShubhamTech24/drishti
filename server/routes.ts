...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Upload media to database for search
  app.post('/api/media/upload', isAuthenticated, upload.single('media'), async (req, res) => {
    try {
      if (!req.file) {
      ...
      // Analyze the media for persons using AI
      const analysisResult = await analyzeSearchMedia(dataUrl, mediaType);
      
      // Save to database
      const mediaRecord = await storage.saveMediaToDatabase({
        filename: file.originalname,
        mediaType,
        ...
        detectedPersons: JSON.stringify(analysisResult.detectedPersons),
        source: 'upload'
      });
      ...
      res.json({
        ...
        detectedPersons: analysisResult.detectedPersons.length,
  ...
  app.get('/api/media', isAuthenticated, async (req, res) => {
    try {
      const media = await storage.getAllStoredMedia();
      const processedMedia = media.map(m => ({
        id: m.id,
        filename: m.filename,
        mediaType: m.mediaType,
        uploadedAt: m.uploadedAt,
        detectedPersons: m.detectedPersons ? JSON.parse(m.detectedPersons).length : 0,
        source: m.source
      }));
  ...
  // Search for lost person in all stored media
  app.post('/api/lost-persons/search-in-database', isAuthenticated, upload.single('targetPerson'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: 'Target person image required' });
      }

      const targetPersonUrl = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
      const allMedia = await storage.getAllStoredMedia();
      
      const searchResults = [];
      
      // Search through all stored media
      for (const media of allMedia) {
        const result = await searchPersonInMedia(media.data, targetPersonUrl, media.mediaType as 'image' | 'video');
        
        if (result.found && result.confidence > 30) { // Minimum confidence threshold
          searchResults.push({
            mediaId: media.id,
      ...
      // Sort by confidence
      searchResults.sort((a, b) => b.searchResult.confidence - a.searchResult.confidence);
      
      res.json({
        totalMediaSearched: allMedia.length,
        matchesFound: searchResults.length,
        results: searchResults
      });
    } catch (error) {
      console.error('Error searching in database:', error);
      res.status(500).json({ message: 'Failed to search in database' });
    }
  });
